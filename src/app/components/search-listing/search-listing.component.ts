  ...
  async searchProducts(filters: any): Promise<void> {
    ...
    this._apiSharedService
      ...
      .subscribe(async (res) => {
        ...
        this.data = await this.productsgroup;
        this.transferState.set(makeStateKey<any>('my-data'), this.data);
        const myData = this.transferState.get<any>(makeStateKey<any>('my-data'), null);
        if (myData) {
          console.log(myData)
          this.productsgroup = myData;
        }
